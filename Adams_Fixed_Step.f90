...
program ddeabm_fixed_step_test
    ...
    type, extends(ddeabm_with_event_class) :: spacecraft
        ...
        integer(kind = 4)  :: num_s  = 1            !! 1: one satellites; 2: twin satellites.
    end type spacecraft
    ...
    type(spacecraft)       :: s
    real(wp), dimension(n) :: x0_1, xf, x, x02
    real(wp)               :: t, z_target
    ...
    !*********************************************************
        subroutine twobody_report(me, t_in, x_in)
            ...
            class is (spacecraft)
                ...
                write(file_unit_report, '(*(f30.15, 1x))') t, x
                ...
                call process(C, S, me%degree + 1, gl%r_earth, 0, me%degree, gl%potential)
