program name
    ...
    integer(kind = 4)                          :: degree = 360
    integer(kind = 4)                          :: i, j
    integer(kind = 4)                          :: raw, col
    real(kind = 8)                             :: c(0: 360, 0: 360)
    real(kind = 8)                             :: s(0: 360, 0: 360)
    real(kind = 8)                             :: position(17280, 3)
    real(kind = 8)                             :: v

    open(unit=11, file='EGM2008.txt')
    do i = 1, (degree + 1) * (degree + 2) / 2
        read(11, *) raw, col, c(raw, col), s(raw, col)
    end do
    close(unit=11)

    open(unit=22, file='coordinates_earth_fixed_system.txt')
    do i = 1, 1
        read(22, *) position(i, :)
        call process(c, s, 360, position(i, :), 0, 10, v)
        write(*, *) v
    end do
    close(unit=22)
...
subroutine Process(C, S, degree, position, n1, n2, V)
    ...
    Call CalculateEF(x, y, z, n2 + 1, E, F)
